...
void control_struct_init(control_struct_t* Control_struct,double* pBuffer, int bufferLength, double inputGain, double* mode_gain, bool enable){
	int i;
	...
	Control_struct->pBuffer=pBuffer;
	Control_struct->bufferLength=bufferLength;
	Control_struct->inputGain=inputGain;
...
static void gst_audio_dynamic_transform_compressor_float(AudioCompressor_t *compressor, float * data, unsigned int num_samples)
{
...
void processing(double input_gain,int mode,int output_mode, double* mode_gain,AudioCompressor_t* audioCompressor){
	...
	gst_audio_dynamic_transform_compressor_float(audioCompressor, (float) sampleBuffer[LEFT_FE_CH], BLOCK_SIZE);
